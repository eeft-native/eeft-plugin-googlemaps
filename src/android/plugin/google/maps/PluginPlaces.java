...
public class PluginPlaces extends CordovaPlugin implements GoogleApiClient.OnConnectionFailedListener {
    private Activity activity;
    public static final Object semaphore = new Object();

    public static final String TAG = "CordovaGoogleMaps";
    ...
    public void initialize(CordovaInterface cordova, CordovaWebView webView) {
	  ...
	  public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
	    ...
	    cordova.getThreadPool().submit(new Runnable() {
	      ...
	      public void run() {
	        try {
	          if ("testFunc".equals(action)) {
	            PluginPlaces.this.testFunc(args, callbackContext);
	          } else if ("hasPermission".equals(action)) {
	            PluginPlaces.this.hasPermission(args, callbackContext);
	          }
	  ...
	  public void hasPermission(final JSONArray args, final CallbackContext callbackContext) throws JSONException {
	    synchronized (semaphore) {
	      ...
	      boolean locationPermission = PermissionChecker.checkSelfPermission(cordova.getActivity().getApplicationContext(), Manifest.permission.ACCESS_FINE_LOCATION) == PermissionChecker.PERMISSION_GRANTED;
	      callbackContext.success(locationPermission ? 1 : 0);
	    }
	  }

    private void testFunc(final JSONArray args, final CallbackContext callbackContext) throws JSONException
    {
      JSONArray result = new JSONArray();
      result.put("Call to the testFunc succeeded");
      callbackContext.success(result);
		}

    private void currentPlace(final CallbackContext callbackContext) throws JSONException {
        if (PermissionChecker.checkSelfPermission(cordova.getActivity(), Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
